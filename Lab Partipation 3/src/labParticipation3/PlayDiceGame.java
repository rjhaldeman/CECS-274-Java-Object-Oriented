...
public class PlayDiceGame {
	...
	public static void main(String[] args) {
		...
		// These lies will create the players used to play the game.
		...
		int loopVariable = 0; //This is my while condition. It won't change.
		...
		while(loopVariable>=0)
		{
			if (answer.equals("1"))
			{
				// These next 2 lines will roll the 4 dice from the 2 players.
				user.rollDices();
				comp.rollDices();
				
				// These next 4 lines will retrieve the value of the 4 dice we just rolled.
				int userRoll1 = user.playerDice[0].getValue();
				int userRoll2 = user.playerDice[1].getValue();
				int compRoll1 = comp.playerDice[0].getValue();
				int compRoll2 = comp.playerDice[1].getValue();	
				
				// These will acquire the 2 totals.
				int userTotal = user.getTotalPoints();
				int compTotal = comp.getTotalPoints();
